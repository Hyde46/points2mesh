...
def tension_loss(pred, positions, gt_positions, placeholders, block_id):
    B = 1
    ...
    knnr = tf.stack([tf.reshape(bv, [1, B*N*K]),
                     tf.reshape(knn, [1, B*N*K])], -1)
...
def laplace_loss(pred1, pred2, placeholders, block_id):
    ...
    laplace_loss = tf.reduce_mean(tf.reduce_sum(
        tf.square(tf.subtract(lap1, lap2)), 1)) * 1500

    move_loss = tf.reduce_mean(tf.reduce_sum(
        tf.square(tf.subtract(pred1, pred2)), 1)) * 100
