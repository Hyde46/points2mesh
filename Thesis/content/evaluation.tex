...
\subsection{Evaluation metrics}
    ...
    calculates completeness, given a ground truth mesh and a prediction by Christopher Batty\footnote{https://github.com/christopherbatty/SDFGen last visited: \today}. Distances are calculated by
    the average distance of points from the ground truth to the prediction.
...
\subsection{Distance metric results}
    ...
    Tables \ref{tab:distance256opt}, \ref{tab:distance1024opt} and \ref{tab:distance7500opt} show the same evaluation of $d_m$ for $\mathcal{N}_{opt}$, though only the mean value
    is indicated for each configuration. As seen in the reconstructions in figures \ref{fig:12}, \ref{fig:11} and \ref{fig:10}, the results are generally worse. The reconstructions
...
\begin{center}
    \captionof{table}{Distance metric evaluations with 256 samples per point cloud for $\mathcal{N}_{recon}$. Calculated with \emph{meshdistance} metric. Only considering configurations $\mathcal{C}_1$ and $\mathcal{C}_2$. Lower values are better.} \label{tab:distance256} 
...
\end{center}
\begin{center}
    \captionof{table}{Distance metric evaluations with 1024 samples per point cloud for $\mathcal{N}_{recon}$. Calculated with \emph{meshdistance} metric. Lower values are better.} \label{tab:distance1024} 
    \begin{tabular}{| l  | c | c | c | c | c | c || c |}
        \hline
        %airplane
        Object& $\mathcal{C}_1$ & $\mathcal{C}_2$ & $\mathcal{C}_3$ & $\mathcal{C}_4$ & BPA & $IFM$ & None\\ \hline
...
\end{center}
\begin{center}
    \captionof{table}{Distance metric evaluations with 7500 samples per point cloud. Calculated with \emph{meshdistance} metric.Only considering configurations $\mathcal{C}_1$ and $\mathcal{C}_2$. Lower values are better.} \label{tab:distance7500} 
...
\begin{center}
    \captionof{table}{Distance metric evaluations with 256 samples per point cloud for $\mathcal{N}_{opt}$. Calculated with \emph{meshdistance} metric. Only considering configurations $\mathcal{C}_1$ and $\mathcal{C}_2$. Lower values are better.} \label{tab:distance256opt} 
...
\end{center}
\begin{center}
    \captionof{table}{Distance metric evaluations with 1024 samples per point cloud for $\mathcal{N}_{opt}$. Calculated with \emph{meshdistance} metric. Lower values are better.} \label{tab:distance1024opt} 
    \begin{tabular}{| l  | c | c | c | c | c | c || c |}
        \hline
        %airplane
        Object& $\mathcal{C}_1$ & $\mathcal{C}_2$ & $\mathcal{C}_3$ & $\mathcal{C}_4$ & BPA & $IFM$ & None\\ \hline
...
\end{center}
\begin{center}
    \captionof{table}{Distance metric evaluations with 7500 samples per point cloud. Calculated with \emph{meshdistance} metric.Only considering configurations $\mathcal{C}_1$ and $\mathcal{C}_2$. Lower values are better.} \label{tab:distance7500opt} 
    \begin{tabular}{| l  | c | c | c | c  | c | c |}
        \hline
        Object& $\mathcal{C}_1$ & $\mathcal{C}_2$ & $\mathcal{C}_3$ &  $\mathcal{C}_4$ & BPA & $IFM$  \\ \hline
...
\subsection{Generalization capabilities}
    ...
    Configurations $\mathcal{C}_{1,2,3,4}$, now are fed one hundred point cloud samples 
    of \emph{bathtub} and \emph{person}, evaluating the distance metric. Even though, none of these configurations have seen any samples of these object categories.
